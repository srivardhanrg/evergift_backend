...
STORYGIFT_SAFARI_ADVENTURE_THEME = StoryTemplate(
    ...
    pages=[
        ...
        PageTemplate(
            ...
            scene_type="wonder",
            realistic_prompt="""Magical underwater scene at golden hour. Child {name} in safari outfit riding on MAKENA's back - massive gentle hippopotamus swimming gracefully underwater. Child holding hippo's back, surrounded by stream of magical golden bubbles from Heartstone Compass. Warm golden sunset light rays piercing down through crystal-clear water surface above, creating cathedral-like light beams that illuminate the child's face brightly and clearly. Natural skin tones on the child's face, face is the bright focal point against the darker water background. Fish swimming around them. Child's face clearly visible, front-facing with eyes wide open in wonder and delight, well-lit by the warm golden light from above. Only child riding hippo, mystical serene two-character composition. Underwater plants swaying gently. Detailed child's amazed expression, face occupying a prominent portion of the frame. Magical underwater cinematography like BBC Blue Planet, soft focus background.""",
